...
const reclameRef = document.querySelector('.reclame');
const closeRef = document.querySelector('.close');

closeRef.addEventListener('click', closeReclame);


function showReclame(){
timoutlId = setTimeout(() =>{
    reclameRef.classList.add('show-reclame');
},3000)
...
function closeReclame(){
    clearTimeout(timoutlId);
    reclameRef.classList.remove('show-reclame');
}